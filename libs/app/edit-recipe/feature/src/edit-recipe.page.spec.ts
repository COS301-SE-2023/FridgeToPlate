...
describe('EditRecipeComponent', () => {
  ...
  it('test delete recipe with valid recipe id', () => {
    ...
    const deleteRecipeSpy = jest.spyOn(
      EditRecipeComponent.prototype,
      'deleteRecipe'
    );
    const storeDispatchSpy = jest.spyOn(TestBed.inject(Store), 'dispatch');
  ...
  it('should only delete the required recipe and not others', () => {
    const mockRecipeList = [
      { recipeId: 'sampleId' },
      { recipeId: 'sampleId2' },
      { recipeId: 'sampleId3' },
    ] as IRecipe[];
    const filteredMockRecipeList = [
      { recipeId: 'sampleId2' },
      { recipeId: 'sampleId3' },
    ] as IRecipe[];
    component.initialize();
    testProfile.createdRecipes = mockRecipeList;
    expect(testProfile.createdRecipes).toStrictEqual(mockRecipeList);
    component.recipeId = 'sampleId';
    component.deleteRecipe();
    component.deleteRecipe();
    expect(component.profile.createdRecipes).toStrictEqual(filteredMockRecipeList);
  });
  ...
  it('test show error message if recipe id is not available', () => {
    ...
    expect(showErrorSpy).toHaveBeenCalledWith(
      new ShowError('Could not delete recipe')
    );
  });
...
describe('Edit Recipe Page', () => {
  ...
  it('should set the name, description, servings, and preparationTime fields', () => {
    const populateFormSpy = jest.spyOn(
      EditRecipeComponent.prototype,
      'populateForm'
    );
    ...
    const initializeSpy = jest
      .spyOn(EditRecipeComponent.prototype, 'initialize')
      .mockImplementation(() => {
    ...
    expect(editRecipePage.recipeForm.value.description).toEqual(
...
describe('Testing Tags', () => {
  ...
  it('should not add a tag if tagValue is empty', () => {
    ...
    expect(dispatchSpy).toHaveBeenCalledWith(
      new ShowError('Please enter valid tag')
    );
  });
  ...
  it('Should not add if tags is already at size three(3)', () => {
    ...
    expect(dispatchSpy).toHaveBeenCalledWith(
      new ShowError('Only a maximum of three tags')
    );
    expect(component.tags).toEqual(testTags);
  });
...
describe('Image upload', () => {
  ...
  it('should update the imageUrl when a file is selected', () => {
    ...
    const readAsDataURLStringSpy = jest.spyOn(
...
describe('isFormValid()', () => {
  ...
  it('Meal Selection', () => {
    ...
    const instructionsFormArray = new FormArray([new FormControl('Step 1')]);
    ...
    component.isFormValid();
    expect(dispatchSpy).toHaveBeenCalledWith(
      new ShowError('Please select a meal')
    );
  });
  ...
  it('Truthy Profile', () => {
    ...
    component.isFormValid();
    expect(dispatchSpy).toHaveBeenCalledWith(
      new ShowError('Please login to create a recipe')
    );
  });
  ...
  it('Form fields validation', () => {
    ...
    component.isFormValid();
    expect(dispatchSpy).toHaveBeenCalledWith(
      new ShowError('Invalid Form. Missing instructions or Ingredient details.')
    );
  });
...
describe('Testing Recipe Update ', () => {
  ...
  it('Should dispatch Update Recipe Action', async () => {
    ...
    component.recipeForm = fb.group({
      ...
      ingredients: fb.array(
        recipe.ingredients.map((ingredient) => fb.control(ingredient))
      ),
      instructions: fb.array(
        recipe.steps.map((instruction) => fb.control(instruction))
      ),
    ...
    expect(dispatchSpy).toHaveBeenCalledWith(
      new Navigate([`/recipe/${recipe.recipeId}`])
    );
  });
  ...
  it('Should dispatch Could not update recipe error', () => {
    ...
    component.updateRecipe();
    expect(dispatchSpy).toHaveBeenCalledWith(
      new ShowError('Could not update recipe')
    );
  });
  ...
  it('Should not create recipe if form is invalid', () => {
    ...
    const profileDataSubject = new BehaviorSubject<IProfile | undefined>(
      undefined
    );
    // Mock the recipe data
    const recipe: IRecipe = {
  ...
  it('should return correct getDifficulty', () => {
    // Arrange
    component.difficulty = 'Medium';

    expect(component.getDifficulty('Medium')).toStrictEqual({
      'bg-primary': true,
      'bg-gray-200': false,
      'text-white': true,
      'text-gray-700': false,
  ...
  it('should return incorrect getDifficulty', () => {
    // Arrange
  ...
  it('should return correct getMealPlan', () => {
    // Arrange
    component.selectedMeal = 'Breakfast';

    expect(component.getMealPlan('Breakfast')).toStrictEqual({
      'bg-primary': true,
      'bg-gray-200': false,
      'text-white': true,
      'text-gray-700': false,
  ...
  it('should return correct getMealPlan', () => {
    ...
    expect(component.getMealPlan('Lunch')).toStrictEqual({
      'bg-primary': false,
      'bg-gray-200': true,
      'text-white': false,
      'text-gray-700': true,
